# Articles & References

Important articles, blog posts, documentation, and references about Claude Code and AI-assisted development.

---

## Official Documentation

### Anthropic Resources
- **[Effective Harnesses for Long-Running Agents](https://www.anthropic.com/engineering/effective-harnesses-for-long-running-agents)** - Official Anthropic blog post on building robust agent systems
- **[Claude Code Documentation](https://code.claude.com/docs)** - Official Claude Code documentation

---

## Blog Posts & Articles

### Ralph Loop Technique
- **[Ralph Wiggum: Claude Code's new way to run autonomously](https://medium.com/@joe.njenga/ralph-wiggum-claude-code-new-way-to-run-autonomously-for-hours-without-drama-095f47fbd467)** by Joe Njenga
  - Comprehensive guide to the Ralph Loop technique
  - Includes PROMPT.md and PRD.json patterns
  - Practical examples and use cases

---

## Video Content

### Setup & Configuration
- **[Boris Cherny Setup Video](https://www.youtube.com/watch?v=AkG-BZ-708U)** by Alex Finn
  - CLAUDE.md instructions setup
  - Best practices configuration

- **[Working with Claude Code Desktop](https://www.youtube.com/watch?v=pZ2N7CJFbBk)** by Alex Finn
  - Creating "CEO" prompts for projects
  - Project management workflows

---

## Community Resources

### Twitter/X Threads
- **[Matt Pocock on Ralph Loop](https://x.com/mattpocockuk/status/2008200878633931247?s=20)** - Insights on autonomous coding patterns
- **[Boris Cherny Chrome Extension Setup](https://x.com/bcherny/status/2007179861115511237?s=20)** - Visual verification workflow

---

## Research Papers

> Add relevant AI/LLM research papers here

---

## Books & Courses

> Add books and courses on AI-assisted development here

---

## Add a New Resource

To add a resource:
1. Choose the appropriate category
2. Include: Title, author, link
3. Add a brief description of what makes it valuable
4. Note any key takeaways or insights
